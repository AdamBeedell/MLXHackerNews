...
class HNTitleDataset(Dataset):
    def __init__(self, data_source, word2idx, domain2idx, snapshot_time):
    ...
    def __getitem__(self, idx):
        ...
        # Feature 2: Domain as index
        domain_idx = self.domain2idx[self.domains[idx]]
        return (
...
def collate_batch(batch):
    idxs, ages, domain_idxs, scores = zip(*batch)
    ...
    ages = torch.cat(ages)  # shape [batch, 1]
    domain_idxs = torch.cat(domain_idxs)  # shape [batch, 1]
    scores = torch.tensor(scores)
    return (pooled, ages, domain_idxs), scores

# ----- Model -----
class Regressor(nn.Module):
    def __init__(self, input_dim, domain_vocab_size, domain_emb_dim=16):
        super().__init__()
        self.domain_emb = nn.Embedding(domain_vocab_size, domain_emb_dim)
        self.net = nn.Sequential(
            nn.Linear(input_dim + 1 + domain_emb_dim, 128),  # +1 for age
            nn.ReLU(),
            nn.Linear(128, 64),
            nn.ReLU(),
            nn.Linear(64, 64),
    ...
    def forward(self, x_tuple):
        x, age, domain_idx = x_tuple  # x is the average-pooled word2vec embedding of the post’s title: [B, input_dim], age: [B, 1], domain_idx: [B, 1] 
        d_emb = self.domain_emb(domain_idx)
        features = torch.cat([x, age.unsqueeze(1), d_emb], dim=1)
...
def train():
    ...
    # Map each domain to a unique index, 0 reserved for unknown
    unique_domains = df['domain'].dropna().unique()
    domain2idx = {d: i+1 for i, d in enumerate(unique_domains)}
    domain2idx_default = defaultdict(zero, domain2idx)

    ds = HNTitleDataset(df, word2idx, domain2idx_default, SNAPSHOT_TIMESTAMP)
    ...
    model = Regressor(EMBED_DIM, len(domain2idx_default)+1)  # +1 for unknown
    model.to(device)
    ...
    for epoch in range(config.epochs):
        ...
        model.train()
        losses = []
        for batch_idx, ((x, age, dom_idx), y) in enumerate(tqdm(loader, desc=f"Epoch {epoch+1}")):
            # Transfer batch to device
            x, age, dom_idx, y = x.to(device), age.to(device), dom_idx.to(device), y.to(device)
